...
**Implementatiestap 5:** het attestation-signaal wordt alleen geëvalueerd op het moment dat een WebAuthn-credential daadwerkelijk wordt aangemaakt (dus tijdens de biometrie-setup uit stap 4) — attestation is achteraf niet opnieuw op te vragen zonder een nieuwe credential. Gebruikers die geen biometrie inschakelen krijgen dus alleen de "aanvullende" signalen (secure-context, Web Crypto, UA-Client-Hints); dat is een bewuste, geaccepteerde beperking van een sowieso al "best effort"-heuristiek, niet een gat dat apart gedicht moest worden. Het attestation-signaal zelf is een byte-lengte-heuristiek op het attestationObject (een `'none'`-attestatie is enkele tientallen bytes; een echte attestatiestatement met certificaat is dat niet) in plaats van een echte CBOR-parse van het `fmt`-veld — voldoende voor "geen garantie", en voorkomt een CBOR-parsing-dependency. "Re-tonen bij re-trigger na update" is geïmplementeerd als: opnieuw tonen zodra er nog geen `SecurityConsentLog`-record voor de huidige `appVersie` bestaat (een nieuwe versie triggert dus altijd opnieuw, ongeacht of de exacte signaalset gelijk is).
...
**Ontdekt tijdens implementatiestap 4:** `authenticatorAttachment: 'platform'` bij het aanmaken van de WebAuthn-credential is slechts een hint, geen garantie — een passkey-beherende browserextensie (Proton Pass, 1Password, Bitwarden e.d.) kan `navigator.credentials.create()`/`.get()` onderscheppen en de credential in de eigen vault opslaan in plaats van in de platform-authenticator. Werkt functioneel gewoon door (`setupBiometric()` faalt alleen als de gekozen provider geen PRF ondersteunt), maar het geheim is dan gebonden aan die extensie-vault, niet aan toestelhardware — een net iets ander dreigingsmodel dan §2 voor "biometrie" veronderstelde. Verandert niets aan de conclusie: de PIN blijft de enige garantie, biometrie (via welke provider dan ook) blijft puur optioneel gemak.
